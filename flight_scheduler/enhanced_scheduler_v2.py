...
try:
    ...
    FUEL_PENALTY = getattr(config, 'FuelPenalty', 5)
...
# Load all data files
flights = pd.read_csv(os.path.join(DATA_DIR, 'enhanced_flights.csv'))
...
certification_matrix = pd.read_csv(os.path.join(DATA_DIR, 'aircraft_crew_certification.csv')).set_index('AircraftID')
gateway_compatibility = pd.read_csv(os.path.join(DATA_DIR, 'gateway_aircraft_compatibility.csv')).set_index('AircraftID')
aircraft_specs = pd.read_csv(os.path.join(DATA_DIR, 'aircraft_specifications.csv')).set_index('AircraftID')
...
# Merge aircraft specs with main aircraft data
aircraft = aircraft.merge(aircraft_specs, on='AircraftID', how='left')

# Model
model = gp.Model("EnhancedFlightSchedulerV2")
...
hazmat_violation = model.addVars(flights.index, vtype=GRB.BINARY, name="hazmat_violation")
volume_violation = model.addVars(flights.index, vtype=GRB.BINARY, name="volume_violation")
...
for f_idx, flight in flights.iterrows():
    ...
    for a_idx, ac in aircraft.iterrows():
        if flight['Volume'] > ac['MaxPayload']:
            model.addConstr(aircraft_used[f_idx, a_idx] <= volume_violation[f_idx], name=f"volume_{f_idx}_{a_idx}")
    
    # 8. Hazmat constraints (simplified - limit hazmat per aircraft)
    if flight['Hazmat'] == 1:
        for a_idx, ac in aircraft.iterrows():
            # Limit hazmat flights per aircraft (simplified constraint)
            hazmat_flights = gp.quicksum(
                aircraft_used[f2_idx, a_idx] * (flights.loc[f2_idx, 'Hazmat'] == 1)
                for f2_idx in flights.index if f2_idx != f_idx)
            model.addConstr(hazmat_flights + aircraft_used[f_idx, a_idx] <= 2, name=f"hazmat_limit_{f_idx}_{a_idx}")

# 9. Aircraft constraints
for a_idx, ac in aircraft.iterrows():
...
model.setObjective(
    ...
    gp.quicksum(HAZMAT_PENALTY * hazmat_violation[f_idx] for f_idx in flights.index) +
    gp.quicksum(VOLUME_PENALTY * volume_violation[f_idx] for f_idx in flights.index),
    GRB.MINIMIZE)

model.optimize()

# Enhanced Output Results
print("\n===== ENHANCED SCHEDULER V2 RESULTS =====")
for f_idx, flight in flights.iterrows():
    ...
    haz_viol = int(hazmat_violation[f_idx].x)
    vol_viol = int(volume_violation[f_idx].x)
    priority = flight['CargoPriorityScore']
    cargo_type = flight['CargoType']
    
    print(f"Flight {flight['FlightNumber']} ({cargo_type}, Priority {priority}) departs at {dep_time}")
    print(f"  Aircraft: {ac_id} ({aircraft.loc[ac_idx, 'Size']}), Crew: {cr_id}")
    print(f"  Violations: SLA={sla_miss}, Gateway={gw_viol}, Hazmat={haz_viol}, Volume={vol_viol}")
...
for f_idx in flights.index:
    ...
    print(f"Flight {flight['FlightNumber']} ({cargo_type}, Priority {priority}):")
    print(f"  Delay: {delay} min, SLA Missed: {sla_miss}")
    print(f"  Gateway Violation: {gw_viol}, Hazmat Violation: {haz_viol}, Volume Violation: {vol_viol}")
...
print("\nAircraft Usage by Size:")
for size in ['Medium', 'Large', 'Jumbo']:
    size_aircraft = [a_idx for a_idx, a in aircraft.iterrows() if a['Size'] == size and a['InMaintenance'] != 'TRUE' and a['MaintenanceDue'] > 0]
    used_size = sum(1 for a_idx in size_aircraft for f_idx in flights.index if aircraft_used[f_idx, a_idx].x > 0.5)
    print(f"{size}: {used_size}/{len(size_aircraft)} aircraft used") 